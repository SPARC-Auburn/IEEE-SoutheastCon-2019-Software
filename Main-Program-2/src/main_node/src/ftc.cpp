...
//void matchStarted(const std_msgs::Int32ConstPtr &msg){
...
int main(int argc, char **argv)
{
 ...
 // initPose = n.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose",1,true);
  ...
  //ros::Subscriber startMatchSub = n.subscribe<std_msgs::Int32>("startMatchFunc", 1, matchStarted);
	...
	*/
	int count = 0;
	bool done = 0;
  double myGoalX[8] = {-0.35,-0.5,-0.35,0,0.35,0.5,0.35,0};
  double myGoalY[8] = {0.35,0,-0.35,-0.5,-0.35,0,0.35,0.5};//set these according to the new map locations
 // double myGoalZ[8] = {-2.36,-1.57,0.785,0.0,0.785,1.571,2.36,0.5};//set these according to the new map locations
    ...
    //this tests the octet and debris goal setting
			//system("roslaunch /home/ubuntu/ieee-2019-electrical-software/launches amcl_diff.launch");
   			ROS_INFO("WE GOT HERE");
			while(count<15){
				moveToGoal(0.5,0, "base_footprint");
				moveToGoal(-0.5,-0.5, "map");
	 			moveToGoal(-0.5,0, "map");
	 			moveToGoal(0.5,-0.5, "map");
	 			moveToGoal(0.5, 0, "map");
	 			moveToGoal(0.5,0.5, "map");
				moveToGoal(-0.5, 0.5, "map");		
